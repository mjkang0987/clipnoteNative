...
### Phase 1 — 코어 (MVP)
- [ ] Expo + expo-router + TS 스캐폴딩, 기본 네비게이션/테마(디자인 토큰).
- [ ] 홈: URL 입력/붙여넣기 → **자동 메타 추출**(디바운스) → 미리보기 카드(웹과 동일 흐름).
- [ ] 공유 카드 미리보기(OG와 동일 구성: 사이트명→제목→설명→ClipNote) + 클립 저장 카드 미리보기.
- [ ] 비로그인 로컬 저장(AsyncStorage).
...
### Phase 3 — 내 클립
- [ ] 목록(`GET /api/clips`) — 카드(썸네일/그라디언트·제목·호스트·태그).
- [ ] 태그 필터, 편집/삭제(웹 API 재사용).
