...
│   ├── faq.tsx         # 자주 묻는 질문
│   └── account/delete.tsx  # 회원 탈퇴(계정·클립 영구 삭제)
├── components/
...
- 2026-06-26: 회원 탈퇴 — 탈퇴 화면(`app/account/delete.tsx`: 경고·삭제 항목 안내·동의 체크 → 확인 Alert → 탈퇴 실행 → 로컬 클립 정리 + 로그아웃 + 홈). `lib/api.deleteAccount` (DELETE `/api/account`, Bearer 토큰), 사이드 메뉴에 로그인 시 "회원 탈퇴" 항목(다른 항목과 같은 16px, 색만 회색으로 약하게), `_layout.tsx` 라우트 등록. tsc 통과.
- 2026-07-02: 회원 탈퇴 서버 엔드포인트 확인 완료 — 웹 백엔드(`app/api/account/route.ts`)에 `DELETE` 핸들러 구현·배포됨(getCurrentUser → clipStore.removeAllByUser → admin.auth.admin.deleteUser → {deleted:true}). 앱 호출 규격(경로·메서드·Bearer·응답)과 일치 확인 → 앱·웹 양쪽 회원 탈퇴 실제 동작. 앱 코드 변경 없음.
- 2026-07-02: Phase 5 공유 인텐트 — 타 앱에서 "공유 → ClipNote" 지원(`expo-share-intent` ^7.0.0). `app.json` config plugin(iOS Share Extension: WebURL·WebPage·Text / Android intent-filter: text/*), `_layout.tsx` `ShareIntentProvider` + 공유 진입 시 홈으로 보내는 `ShareIntentGate`, `app/index.tsx` 가 `shareIntent.webUrl`(폴백 text)을 URL 입력란에 채워 자동 메타 추출로 연결. ⚠️ 네이티브 변경 → dev build 재생성 필요(Expo Go 불가). tsc 통과.
