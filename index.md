...
- 2026-06-26: 회원 탈퇴 — 탈퇴 화면(`app/account/delete.tsx`: 경고·삭제 항목 안내·동의 체크 → 확인 Alert → 탈퇴 실행 → 로컬 클립 정리 + 로그아웃 + 홈). `lib/api.deleteAccount` (DELETE `/api/account`, Bearer 토큰), 사이드 메뉴에 로그인 시 "회원 탈퇴"(danger) 항목, `_layout.tsx` 라우트 등록. ⚠️ 서버 `DELETE /api/account`(service role 로 auth 유저+소유 클립 삭제) 엔드포인트가 웹 백엔드에 추가돼야 동작. tsc 통과.
